...
ggplot(temp, 
       aes(x=delta, y=value, col=factor(SubjectID))) + geom_line() + geom_point()
```
...
temp = alsfrs %>%
  filter(feature == "ALSFRS_R_Total")
ggplot(temp, aes(x=delta, y=value, col=factor(SubjectID))) + geom_line() + theme(legend.position = "none")
```
